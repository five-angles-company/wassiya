...
> **Amended 2026-09-25, deliberately: escrow is one key, held in the Convex
> env.** Each routed item's key is sealed on the owner's phone to an X25519
> escrow key; its secret is an env var on the deployment. The per-heir bundle
> (K_h, RSA, Google Cloud KMS) is gone: it copied server-supplied routing, went
> stale, and protected nothing the database did not already decide. **The trade
> must not be hidden: whoever holds admin on the production Convex deployment
> can open every routed item of every owner, alive or not.** For routed items
> the protection is the gate's code, the audit log and account security — not
> cryptography. Unrouted items open for no one. Offered and declined: the
> escrow secret kept offline with a staff unlock per death (removes that risk,
> costs a manual step). Earlier (2026-09-19) the guardian was removed — a second
> human holding half of every heir's key never worked end to end. Do not bring
> back guardians, bundles or 2-of-2 language.
>
...
- Per-asset: random DEK (XChaCha20-Poly1305) wrapped by MK. The label and the secret fields are sealed under it on the row (`labelSealed`, `secretSealed`); each file, and its thumbnail, is encrypted client-side as its own blob (`files`). An edit reuses the DEK — a new one would strand its escrowed copy.
- Rotation: a new paper sheet ⇒ regenerate S_paper, bump `paperVersion`, re-wrap.

### Escrowed release
- **Heirs NEVER receive MK.** When an asset is routed to anyone, the owner's device seals its DEK to the escrow public key (`sealForEscrow`, bound to `ownerId|assetId`) in the same `routing.setRecipients` call; routing to nobody deletes the sealed copy. A personal message key is sealed the same way, bound to `ownerId|heirId`. Only routed items are ever escrowed; everything else dies with the owner. Moving an item between heirs changes no key.
- **The escrow public key is pinned in the mobile app** (`lib/escrow-key.ts`, per environment) and is never served by the backend. A served key would let a compromised server substitute its own and receive every routed key. The backend refuses a seal whose key id is not `ESCROW_KEY_ID`.
- **One unlock path.** `escrow.openDelivery` is the only reader of `escrowedDek`, `messageKeyEscrowed` and `ESCROW_PRIVATE_KEY`, and the only server module that imports `@workspace/crypto`. It is a mutation: it re-asserts every precondition itself (claim `released` with `nameMatch`, delivery `ready`, caller is the bound heir, caller Didit-verified, not expired), audits the open in the same transaction, opens only this heir's keys and returns them with the items. The heir's browser decrypts everything itself. `pnpm --filter @workspace/backend verify` enforces all of it.
- **Escrow key:** the X25519 secret is `ESCROW_PRIVATE_KEY` in the deployment env, one per environment. A `dev*` key id is refused when `WASSIYA_ENV=production` — a development shortcut must never silently ship. **Keep two offline copies of the production secret:** lost, it means no heir ever receives anything. Rotation is not built: a new key id strands every item sealed under the old one until an owner's device re-seals it.
- **Release gates, in order:** death certificate + staff name match against the owner's verified legal name → veto window (the owner's fingerprint check-in stops it) → claim `released`, which closes the vault → per-heir delivery → heir Didit verification matched to the heir record → unlock.
...
- **A released vault is closed** (`users.vaultClosedAt`): `keyring.get` hands out no recovery wrapper, `keyring.save` and new-device `devices.register` refuse. Without that, whoever finds the sheet after a death could recover everything, including what the owner let die with them. The one way back, for an owner proven alive, is `npx convex run claims:reopenVault`, which also stops every open delivery.
- **Delivery window: one year from release.** When the owner's last delivery closes, the whole vault is deleted (`vault.purge`: assets, files, routing, messages, recovery wrapper) and nobody, Wassiya included, can open any of it again.
- Rule for all code: no plaintext key material in Convex functions (outside `escrow.openDelivery`'s memory and its return to the verified heir), logs, or errors. OTP/Clerk auth proves identity only — it never touches keys.

## Actors and apps (LOCKED)
- **Owner → mobile only.** MK, biometrics and the check-in (which is also the veto) need a hardware keystore and a fingerprint. Never move any of them to a browser.
- **Heir → web only. Death reporter → web only.** Never add an heir screen to `apps/mobile`; mobile is the owner's app.
- **A death report (`claims`) is about the owner; a delivery (`deliveries`) is about one heir.** Anyone may file a report and attach the certificate. The filer receives nothing by filing. When a report reaches `released`, one delivery is created per heir who receives anything, and **Wassiya contacts each heir** on the owner-registered phone with a link. Never re-merge the two: a report's author is not its recipient.
...
- Subscription lapse: vault stays readable and heir delivery keeps working; only adding assets is blocked. There is **no grace period** — a `renewsAt` in the past blocks the next add and nothing else.
...
- Identity verification (Didit) is mandatory and blocking for owners at onboarding, and for heirs before any delivery opens; the death certificate name must match the owner's verified legal name. **The person who reports a death is never asked to verify** — they receive nothing, and the certificate, the veto window and each heir's own verification guard the release.
