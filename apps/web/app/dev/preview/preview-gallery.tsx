...
import { caseView, type CaseFacts } from "@/features/claims/lib/case-view"
...
const STATES: { name: string; facts: Omit<CaseFacts, "submittedAt"> }[] = [
  ...
  return (
    <div className="flex flex-col gap-24">
      ...
      {STATES.map(({ name, facts }) => {
        const view = caseView({ ...facts, submittedAt: NOW - 14 * DAY }, locale)
        return (
      ...
      <section id="gate">
        <SheetGate expiresAt={NOW + 340 * DAY} sheets={{ executor: true, recovery: true }} onSubmit={() => "unreadable"} />
      </section>

      <section id="gate-recovery-only">
        <SheetGate expiresAt={NOW + 340 * DAY} sheets={{ executor: false, recovery: true }} onSubmit={() => "wrongSheet"} />
      </section>
