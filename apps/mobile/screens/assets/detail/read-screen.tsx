/**
 * One asset, as a read-only grouped row list.
 *
 * ## Interim: this is what the not-yet-converted types still get
 *
 * ٤.٩ is becoming an *edit* screen — tapping an asset opens its form. The
 * digital account has moved (`edit-screen.tsx`); bank, crypto, note, document
 * and photos have not, and each needs work this screen does not: IBAN
 * validation, a legacy phrase payload that stores neither its network nor its
 * kind, a note draft store that must not be prefilled with plaintext, and two
 * file pickers. They keep this view until they land, rather than being handed a
 * form that cannot save them.
 *
 * Everything below is unchanged from when it was the whole screen.
 *
 * ## Why this shape
 *
...
export function AssetReadScreen({ assetId }: { assetId: Id<"assets"> }) {
