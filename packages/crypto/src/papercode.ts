/**
 * A printed sheet's 32-byte secret rendered as something a person can copy off
 * paper under stress and a phone keyboard can accept without a numeric row.
 *
 *   WSY1-XXXX-XXXX-…    the owner's recovery sheet   (prefix + version,
 *   WSE1-XXXX-XXXX-…    an executor's sheet           then 14 groups of 4)
 *
...
/**
 * Two kinds of printed sheet share this format. The prefix is only a label —
 * it sits outside the checksum — so `domain` is what really separates them: it
 * is folded into the CRC and never printed, and a code minted for one kind fails
 * the other's checksum exactly like a typo. The recovery sheet's domain is
 * empty, which is the format every sheet in a drawer was printed with.
 */
type CodeFormat = { prefix: string; domain: Uint8Array; label: string }

const RECOVERY: CodeFormat = {
  prefix: "WSY",
  domain: new Uint8Array(0),
  label: "Recovery code",
}
const EXECUTOR: CodeFormat = {
  prefix: "WSE",
  domain: utf8ToBytes("wassiya/executor-sheet"),
  label: "Executor code",
}
...
export type DecodedExecutorCode = { sheetSecret: Uint8Array; version: number }
...
export function encodePaperCode(sPaper: Uint8Array, version: number): string {
  return encodeCode(RECOVERY, sPaper, version)
}

export function decodePaperCode(code: string): DecodedPaperCode {
  const { secret, version } = decodeCode(RECOVERY, code)
  return { sPaper: secret, version }
}

/** An executor's sheet. `version` is that executor's `sheetVersion`. */
export function encodeExecutorCode(sheetSecret: Uint8Array, version: number): string {
  return encodeCode(EXECUTOR, sheetSecret, version)
}

export function decodeExecutorCode(code: string): DecodedExecutorCode {
  const { secret, version } = decodeCode(EXECUTOR, code)
  return { sheetSecret: secret, version }
}

/** Which kind of sheet a typed code claims to be, by its prefix alone. */
export function paperCodeKind(code: string): "recovery" | "executor" | null {
  const normalised = code.replace(/[\s-]/g, "").toUpperCase()
  if (normalised.startsWith(EXECUTOR.prefix)) return "executor"
  if (normalised.startsWith(RECOVERY.prefix)) return "recovery"
  return null
}

/**
 * `version` appears both in the human-readable prefix and inside the
 * checksummed payload, and the decoder rejects a code whose two copies disagree.
 */
function encodeCode(format: CodeFormat, secret: Uint8Array, version: number): string {
...
/**
 * Hyphens, spaces and case are all forgiven; anything else is a hard error.
 * Errors quote positions and lengths, never characters — this string is key
 * material and must not reach a log through a message.
 */
function decodeCode(
  format: CodeFormat,
  code: string
): { secret: Uint8Array; version: number } {
  const normalised = code.replace(/[\s-]/g, "").toUpperCase()
  ...
  if (!/^\d+$/.test(versionText)) {
    throw new Error(`${format.label} must be ${format.prefix}<version> followed by the code`)
  }
  ...
  if (crc16(concatBytes(format.domain, body)) !== checksum) {
    throw new Error(`${format.label} checksum failed — check for a mistyped character`)
  }
