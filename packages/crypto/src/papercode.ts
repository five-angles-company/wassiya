/**
 * The printed recovery sheet: S_paper rendered as something a person can copy
 * off paper under stress and a phone keyboard can accept without a numeric row.
 *
 *   WSY1-XXXX-XXXX-…   (prefix + 14 groups of 4)
 *
 * Payload is 35 bytes — version(1) ‖ S_paper(32) ‖ CRC-16(2) — which is exactly
...
/**
 * Render the paper share. `version` is the keyring's `paperVersion`; it appears
 * both in the human-readable prefix and inside the checksummed payload, and the
 * decoder rejects a code whose two copies disagree.
 */
...
/**
 * Parse a typed-in sheet. Hyphens, spaces and case are all forgiven; anything
...
export function decodePaperCode(code: string): DecodedPaperCode {
  ...
  if (rest.length !== CODE_CHARS + 1 && rest.length !== CODE_CHARS + 2) {
    throw new Error(
      `Recovery code must be ${PREFIX}<version> plus ${CODE_CHARS} characters, received ${rest.length} after the prefix`
    )
  }
  ...
  if (!/^\d+$/.test(versionText)) {
    throw new Error(
      `Recovery code must be ${PREFIX}<version> followed by the code`
    )
  }
  ...
  if (crc16(body) !== checksum) {
    throw new Error(
      "Recovery code checksum failed — check for a mistyped character"
    )
  }

  const version = body[0]!
  if (version !== prefixVersion) {
    throw new Error("Recovery code version does not match its prefix")
  }
...
function assertVersion(version: number): void {
  if (!Number.isSafeInteger(version) || version < 1 || version > 99) {
    throw new Error("Paper code version must be an integer between 1 and 99")
  }
}
...
function base32Decode(encoded: string): Uint8Array {
  ...
  for (let i = 0; i < encoded.length; i++) {
    ...
    if (value < 0) {
      throw new Error(
        `Recovery code contains an unusable character at position ${i + 1}`
      )
    }
